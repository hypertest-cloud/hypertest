...
const getE2eFilePath = (options: CypressPluginOptions) =>
  `${options.projectPath}//cypress//support//e2e.ts`;
const getConfigFilePath = (options: CypressPluginOptions) =>
  `${options.projectPath}//cypress.config.ts`;

export const overrideItCallback = (options: CypressPluginOptions) =>
...
export const removeItCallbackOverride = (options: { projectPath: string }) =>
  new Promise<void>((resolve, reject) => {
    const e2eFilePath = getE2eFilePath(options);
    ...
    fs.readFile(e2eFilePath, 'utf8', (error, modifiedData) => {
      ...
      const separatorIndex = modifiedData.lastIndexOf(
        HYPERTEST_FILE_MODIFICATION_SEPARATOR,
      );
      if (separatorIndex === -1) {
      ...
      const modificationRemovedData = modifiedData
    ...
    fs.readFile(configFilePath, 'utf8', (error, modifiedData) => {
      ...
      const endIndex = modifiedData.indexOf(
        CUSTOM_LOGGER_CONFIG_END_MARK,
        startIndex,
      );
      if (endIndex === -1) {
      ...
      const afterInsertedCode = modifiedData
        .slice(endIndex + CUSTOM_LOGGER_CONFIG_END_MARK.length)
        .trimStart();
