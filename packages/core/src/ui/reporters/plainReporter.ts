...
const handleInvokeEvent = (event: HypertestEvent) => {
  if (event.type === 'run:start') {
    write(
      `[run:start] ${event.testCount} tests · concurrency ${event.concurrency} · run ${event.runId}`,
    );
    ...
    write(
      `[run:end] ${t.total} tests · ${t.success} passed · ${t.skipped} skipped · ${t.failed} failed`,
    );
  }
};
...
const handleDeployEvent = (event: HypertestEvent) => {
  ...
  if (event.status === 'start') {
    ...
    write(
      `[deploy] ${event.step} done (${((event.durationMs ?? 0) / 1000).toFixed(1)}s)`,
    );
  } else {
