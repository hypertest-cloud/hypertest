...
export const Plugin = (options: PlaywrightPluginOptions): HypertestPlugin<PlaywrightCloudFunctionContext> => ({
  ...
  buildImage: async () => {
    try {
      ...
      console.log('Creating Dockerfile for user project based on hypertest.config.ts...')
      await fs.mkdir('.hypertest/', {
        recursive: true
      })
      const config = await import(`${process.cwd()}/hypertest.config.js`).then(mod => mod.default)
